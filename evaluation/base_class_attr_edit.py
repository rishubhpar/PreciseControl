...
class GeneratedDataset(Dataset):
    def __init__(self,
                 eval_folder: str,
                 prompt_file: str = './infer_images/comparison_attr_edit.txt',
    ...
    def _load_txt_files(self):
        ...
        for identity in identity_list:
            for prompt in prompt_once:
                file_name = os.listdir(os.path.join(self.eval_folder, identity, "original"))
                src_img_paths.append(
                    [os.path.join(self.eval_folder, identity, "original", file_name[0])]
                )
        ...
        for identity in identity_list:
            for prompt in prompt_once:
                ...
                file_name = os.listdir(os.path.join(self.eval_folder, identity, self.attr))
                gen_img_folders.append(
                    os.path.join(self.eval_folder,identity, self.attr)
                )
...
class IDCLIPScoreCalculator(ABC):
    ...
    def start_calc(self, include_undetected_samples: bool = False):
        ...
        print(" Evaluation is done for attr: ", self.attr)
