...
class GeneratedDataset(Dataset):
    def __init__(self,
        ...
        self.trans = functools.partial(processor, return_tensors="pt")

        # from facenet_pytorch import MTCNN
        # self.mtcnn = MTCNN(image_size=512, select_largest=False, keep_all=True)
...
#         src_img_tensor = torch.tensor(np.asarray(src_img))
#         gen_img = Image.open(os.path.join(gen_img_folder, os.listdir(gen_img_folder)[0])).convert('RGB') 
#         gen_img_tensor = torch.tensor(np.asarray(gen_img))

#         gen_img_tensors = gen_img_tensor.unsqueeze(0)
...
#######################################
    ...
    #     src_img = Image.open(src_img_path).convert('RGB')
    #     src_boxes, _ = self.mtcnn.detect(src_img)
    #     src_no_face = src_boxes is None
    #     if not src_no_face:
    #         src_face = src_img.crop(src_boxes[0])
    #         src_face.save("/mnt/data/rishubh/sachi/src.jpg")
    #         src_img_tensor = self.trans(src_face)["pixel_values"]  
    #     else:
    #         src_img_tensor = torch.zeros((3,224,224))


    #     gen_img = Image.open(os.path.join(gen_img_folder, os.listdir(gen_img_folder)[0])).convert('RGB') 
    #     gen_boxes, _ = self.mtcnn.detect(gen_img)
    #     gen_no_face = gen_boxes is None
    #     if not gen_no_face:
    #         gen_face = src_img.crop(gen_boxes[0])
    #         gen_face.save("/mnt/data/rishubh/sachi/gen.jpg")
    #         gen_img_tensor = self.trans(gen_face)["pixel_values"]
    #     else:
    #         gen_img_tensor = torch.zeros((3,224,224))
    ...
    #     if (not src_no_face) and (not gen_no_face):
    #         has_face = 2
    #     elif (src_no_face) and (not gen_no_face):
    #         has_face = 1
    #     elif (not src_no_face) and (gen_no_face):
    #         has_face = 1
    #     elif (src_no_face) and (gen_no_face):
    #         has_face = 0
...
#######################################
    ...
    #     src_img = Image.open(src_img_path).convert('RGB')
    #     src_img.save("/mnt/data/rishubh/sachi/src.jpg")
    #     src_img_tensor = self.trans(src_img)["pixel_values"]  
        
    #     gen_img = Image.open(os.path.join(gen_img_folder, os.listdir(gen_img_folder)[0])).convert('RGB') 
    #     gen_img.save("/mnt/data/rishubh/sachi/gen.jpg")
    #     gen_img_tensor = self.trans(gen_img)["pixel_values"]
...
class IDCLIPScoreCalculator(ABC):
    ...
    def _check_lmk_box_for_one_image(self, img_size, img_arr: np.ndarray):
        full_img = img_arr.astype(np.uint8)
        lmks = demo_image(full_img, self.net, self.detector)
        if len(lmks) > 0:
            lmk = get_5_from_98(lmks[0])
            cropped_img = norm_crop(full_img, lmk, img_size, mode=self.align_mode, borderValue=0.0)
    ...
    def start_calc(self, include_undetected_samples: bool = False):
        ...
        for data in self.data_loader:
            ...
            src_img = data['src_img_tensor'].squeeze(1)
            gen_img = data['gen_img_tensors'].squeeze(1).squeeze(dim=0)

            import pdb;pdb.set_trace()

            cropped1, success1 = self._check_lmk_box_for_one_image(1024, np.asarray(src_img).astype(np.uint8))
            src_face = PIL.Image.fromarray(numpy.uint8(cropped1).astype(np.uint8))
            src_face.save("/mnt/data/rishubh/sachi/src.jpg")
            src_img_tensor = self.trans(src_face)["pixel_values"]  
            
            cropped2, success2 = self._check_lmk_box_for_one_image(512, np.asarray(gen_img).astype(np.uint8))
            gen_face = PIL.Image.fromarray(numpy.uint8(cropped2).astype(np.uint8))
            gen_face.save("/mnt/data/rishubh/sachi/gen.jpg")
            gen_img_tensor = self.trans(gen_face)["pixel_values"]  

            if (success1) and (success2):
                has_face = 2
            elif (success1) and (not success1):
                has_face = 1
            ...
            has_face = data['has_face'][0]
            no_face = data['no_face'][0]
