...
from evaluation.base_class_dino import EvaluatorBase
...
class IdentityEvaluator(object):
    def __init__(self,
        ...
        self.id_model = AutoModel.from_pretrained("facebook/dinov2-base").to(device)
    ...
    def start_calc(self, ori1: torch.Tensor, ori2: torch.Tensor):
        ...
        ################## For Dinov2 Identity evaluation
        faces = torch.cat([ori1, ori2], dim=0)
        feats = self.id_model(faces).last_hidden_state.mean(dim=1)
        # import pdb;pdb.set_trace()
        feat1 = F.normalize(feats[:n1], dim=-1, p=2)
        feat2 = F.normalize(feats[n1:], dim=-1, p=2)
