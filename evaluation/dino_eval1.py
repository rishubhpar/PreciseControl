...
from evaluation.arcface.util import load_model as arcface_model
...
class IdentityEvaluator(object):
    def __init__(self,
        ...
        self.trans_dino = functools.partial(processor, return_tensors="pt")

        from transformers import AutoModel
        self.id_model_dino = AutoModel.from_pretrained("facebook/dinov2-base").to(device)
    ...
    def start_calc(self, ori1: torch.Tensor, ori2: torch.Tensor):
        ...
        # plot the crops images
        # for i in range(crops.shape[0]):
        #     print(crops[i].shape, crops.shape)
        #     img = crops[i].permute(1, 2, 0).cpu().numpy()
        #     img = ((img + 1.) * 127.5).astype(np.uint8)
        #     img = Image.fromarray(img)
        #     img.save(f"./crops_{i}.jpg")
        crop1 = crops[:n1]
        crop2 = crops[n1:]
        avg_cos_sim, avg_mse_dist, avg_l2_dist = self._img_to_img_id_sim_dino(crop1, crop2)
    ...
    def _check_lmk_box_for_tensor(self, img_tensor: torch.Tensor):
        ...
        for i in range(b):
            ...
            if success or i == 0:
                ...
                ########
                img_cropped.append(self.trans_dino(Image.fromarray(np.uint8(cropped)))["pixel_values"] .to(img_tensor.device))
                ########
            else:
                num_no_face += 1
        img_cropped = torch.stack(img_cropped, dim=0).squeeze(1)
    ...
    def _img_to_img_id_sim(self, face1: torch.Tensor, face2: torch.Tensor):
        ...
        # feats = self.id_model.extract_feats(faces)
    ...
    def _img_to_img_id_sim_dino(self, face1: torch.Tensor, face2: torch.Tensor):
        n1, c, h, w = face1.shape
        n2, c, h, w = face2.shape
        if n1 < 1 or n2 < 1:
            return 0, 0, 0
        ...
        # feats = self.id_model.extract_feats(faces)
        feats = self.id_model_dino(faces).last_hidden_state.mean(dim=1)
    ...
    def _load_fr_net(self):
        ...
        # id_path = './weights/cosface_eval/backbone.pth'
        self.id_model = cosface.net.sphere().cuda()
        id_path = './weights/cosface_eval/net_sphere20_data_vggface2_acc_9955.pth'
        weights = torch.load(id_path)
        self.id_model.load_state_dict(weights)
        # import pdb;pdb.set_trace()

        # self.id_model = IDLoss("./weights/model_ir_se50.pth")
        # self.id_model = arcface_model("resnet-101", "./weights/model_0000.pth")
