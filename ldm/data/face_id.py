...
class FaceIdDatasetStyleGAN3(Dataset):
    def __init__(self,
        ...
        print("specific Ids :", specific_ids)
        ''' full image list '''
        self.pickle_path = pickle_path
        self.num_ids = num_ids
        self.specific_ids = specific_ids
    ...
    def _load_from_pickle(self):
        ...
        for id, images in pickle_dict.items():
            ...
            if id == list(pickle_dict.keys())[-1] and use_id < self.num_ids:
                raise ValueError('Reach last. Not enough images for num_ids=%d, only %d.' % (
                    self.num_ids, len(train_id_set)))
        print("img_dict :", self.img_dict)
        print("train set :", train_id_set)
        ''' 2. load reg '''
        reg_cnt = 0
        for id, images in pickle_dict.items():
            if len(self.img_dict.keys()) >= self.num_ids + self.reg_ids:
            ...
            if id == list(pickle_dict.keys())[-1]:
                raise ValueError('Reach last. Not enough images for reg_ids=%d, only %d.' % (
                    self.reg_ids, reg_cnt))
        print("img_dict after reg :", self.img_dict)
        print("train set after reg :", train_id_set)
    ...
    def __getitem__(self, i):
        ...
        if dual_img:  # (1,diff,1+diff)
            ...
            example["image_ori"] = {'faces': torch.cat([img.permute(1, 2, 0)]
                                    ...
                                    'num_ids': 1}
            img = self._add_bg(img, None) if not self.split == 'dev' else img
        # print("[faces] :", example["image_ori"]['faces'].shape)
        # print("[img2_list] [img3_list] :", len(img2_list), len(img3_list))
...
class FFhq_dataset(Dataset):
    def __init__(self,root_dir,split='train',image_size=256, limit_dataset_size=None, use_aug=False, use_data_interpolation=False,ffhq_latents_path=None,
                 percentage_of_synthetic_data=1.0, use_two_ids=False, lora_finetuning=False, multiple_samples=False):
        super(FFhq_dataset, self).__init__()
        self.root_dir = root_dir
        self.split = split
        self.image_size = image_size
        self.use_aug = use_aug
        self.aligned_dataset = False
        self.use_data_interpolation = use_data_interpolation
        self.percentage_of_synthetic_data = percentage_of_synthetic_data
        ...
        if(ffhq_latents_path!=None):
            self.ffhq_latents = torch.load(ffhq_latents_path).detach().cpu()
            print("[Num of latents] : %d"%(len(self.ffhq_latents)))
        else:
            self.ffhq_latents = None
        self.use_two_ids = use_two_ids
        self.multiple_samples = multiple_samples

        self.img_list = []
        self.get_img_list()
        if(split=='train'):
            self.img_list = self.img_list[:68000] if len(self.img_list) > 68000 else self.img_list
            self.ffhq_latents = self.ffhq_latents[:68000] if self.ffhq_latents is not None else None
            if(lora_finetuning):
                self.ori_img_list = sorted(self.img_list)
                self.img_list = self.img_list * 2000
        elif(split=='val'):
            self.img_list = self.img_list[-50:] if len(self.img_list) > 50 else self.img_list
            self.ffhq_latents = self.ffhq_latents[-50:] if self.ffhq_latents is not None else None
        if(limit_dataset_size!=-1):
            self.img_list = self.img_list[:limit_dataset_size]
            self.ffhq_latents = self.ffhq_latents[:limit_dataset_size] if self.ffhq_latents is not None else None
        print("[Num of images in %s set] : %d"%(split,len(self.img_list)))
    ...
    def get_img_list(self):
        for folder in sorted(os.listdir(self.root_dir)):
            if(folder.startswith('0')):
                for img in sorted(os.listdir(os.path.join(self.root_dir,folder))):
                    self.img_list.append(os.path.join(self.root_dir,folder,img))
    ...
    def __getitem__(self,idx):
        ...
        # TODO: check alignment code, sometimes getting no landmark detected, check should we switch img to grey scale.
        # getting aligned face image to get better w latent, based on inference code in e4e
        aligned_face = align_face(self.img_list[idx],self.shape_predictor)
        while(aligned_face==None):
            idx = np.random.randint(len(self.img_list))
            aligned_face = align_face(self.img_list[idx],self.shape_predictor)
            
        aligned_face = self.transform(aligned_face)
        if(self.ffhq_latents!=None):
            example["wlatent1"] = self.ffhq_latents[idx]

        if(self.use_data_interpolation):
            idx2 = np.random.randint(len(self.img_list))
            aligned_face2 = align_face(self.img_list[idx2],self.shape_predictor)

            while(aligned_face2==None or idx2==idx):
                idx2 = np.random.randint(len(self.img_list))
                if(idx2==idx):
                    continue
                aligned_face2 = align_face(self.img_list[idx2],self.shape_predictor)
            aligned_face2 = self.transform(aligned_face2)
            
            
            example["aligned_faces2"] = aligned_face2.permute(1,2,0)
            example["image_size"] = self.image_size

            if(self.ffhq_latents is not None):
                example["wlatent2"] = self.ffhq_latents[idx2]
            example["use_dataset_interpolation"] = self.use_data_interpolation
            example["percentage_of_synthetic_data"] = self.percentage_of_synthetic_data
        else:
            if(self.use_two_ids):
                idx2 = np.random.randint(len(self.img_list))
                aligned_face2 = align_face(self.img_list[idx2],self.shape_predictor)
                while(aligned_face2==None):
                    idx2 = np.random.randint(len(self.img_list))
                    aligned_face2 = align_face(self.img_list[idx2],self.shape_predictor)
                image2 = Image.open(self.img_list[idx2]).convert('RGB')
                image2 = self.transform(image2)
                aligned_face2 = self.transform(aligned_face2)
                example["aligned_faces"] = torch.cat([aligned_face.permute(1,2,0)]+[aligned_face2.permute(1,2,0)],dim=-1)
                if(self.ffhq_latents is not None):
                    example["wlatent2"] = self.ffhq_latents[idx2]
                img = self.transform(img)
                img_final = self._mix_two_tensors(img, image2, None)


            if(self.ffhq_latents is not None):
                example["wlatent2"] = self.ffhq_latents[idx]
            example["aligned_faces2"] = aligned_face.permute(1,2,0)
            example["image_size"] = self.image_size
            example["use_dataset_interpolation"] = False
            example["percentage_of_synthetic_data"] = self.percentage_of_synthetic_data

        # # for multiple identity
        # example["aligned_faces"] = torch.cat([aligned_face.permute(1,2,0)]*2,dim=-1)

        # img = self.transform(img)
        # example["image"] = img.permute(1,2,0)
        # # TODO: check the augmentation they are using and figure out whether useful in this case
        # example["image_ori"] = {'faces': torch.cat([img.permute(1, 2, 0)]*2,dim=-1),  # (N,H,W,C)
        #                         'ids': torch.tensor([0]*2),  # (N,1)
        #                         'num_ids': 1}


        if(not self.use_two_ids):
            
            example["aligned_faces"] = aligned_face.permute(1,2,0)

            img = self.transform(img)
            example["image"] = img.permute(1,2,0)
            # TODO: check the augmentation they are using and figure out whether useful in this case
            example["image_ori"] = {'faces': img.permute(1, 2, 0),  # (N,H,W,C)
                                    'ids': torch.tensor([0]),  # (N,1)
                                    'num_ids': 1,
                                    # 'w_latent': w_latent,
                                    }
            if(self.multiple_samples):
                multiple_samples = []
                for img_id in range(len(self.ori_img_list)):
                    multiple_samples.append(self.transform(Image.open(self.ori_img_list[img_id]).convert('RGB')).permute(1,2,0).unsqueeze(0).cpu().numpy())
                example["img_list"] = np.concatenate(np.array(multiple_samples),axis=0)
        else:
            # img = self.transform(img)
            example["image"] = img_final.permute(1,2,0)
            example["image_ori"] = {'faces': torch.cat([img.permute(1, 2, 0)] + [image2.permute(1, 2, 0)],dim=-1),  # (N,H,W,C)
                                    'ids': torch.tensor([0,0]),  # (N,1)
                                    'num_ids': 2,
                                    # 'w_latent': w_latent,
                                    }
        ''' text '''
...
class Stylegan_human_dataset(Dataset):
    def __init__(self,root_dir,split='train',image_size=256, limit_dataset_size=None, use_aug=False, use_data_interpolation=False,ffhq_latents_path=None,
                 percentage_of_synthetic_data=1.0, use_two_ids=False):
        super(Stylegan_human_dataset, self).__init__()
        self.root_dir = root_dir
        self.split = split
        self.image_size = image_size
        self.use_aug = use_aug
        ...
        self.get_img_list()
        if(split=='train'):
            self.img_list = self.img_list[:68000]
            self.ffhq_latents = self.ffhq_latents[:68000] if self.ffhq_latents is not None else None
        elif(split=='val'):
            self.img_list = self.img_list[-50:]
            self.ffhq_latents = self.ffhq_latents[-50:] if self.ffhq_latents is not None else None
        if(limit_dataset_size!=-1 and split=='train'):
            self.img_list = self.img_list[:limit_dataset_size]
            self.ffhq_latents = self.ffhq_latents[:limit_dataset_size] if self.ffhq_latents is not None else None
        print("[Num of images in %s set] : %d"%(split,len(self.img_list)))
    ...
    def __getitem__(self,idx):
        ...
        if(self.use_data_interpolation):
            idx2 = np.random.randint(len(self.img_list))
            # aligned_face2 = align_face(self.img_list[idx2],self.shape_predictor)
            aligned_face2 = Image.open(self.img_list[idx2]).convert("rgb")
        ...
        background_img = torch.ones((3,512,512))
        img = self.diff_transform(img)
        background_img[:,:,128:384] = img
        img = background_img
        example["image"] = img.permute(1,2,0)
        # TODO: check the augmentation they are using and figure out whether useful in this case
        example["image_ori"] = {'faces': img.permute(1, 2, 0),  # (N,H,W,C)
        ...
        ''' text '''
        placeholder_string = per_img_token_list[0]
        text = random.choice(imagenet_templates_small).format('person wearing %s dress' % placeholder_string)
        example["caption"] = text
