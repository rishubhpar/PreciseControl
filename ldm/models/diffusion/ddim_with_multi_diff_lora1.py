...
def get_face_bounding_boxes(face_img):
    ...
    # mask_json = get_sam_mask(face_img)
    mask1 = torch.from_numpy(np.array(mask_json[1]["mask"]))
    mask2 = torch.from_numpy(np.array(mask_json[2]["mask"]))
    mask3 = torch.from_numpy(np.array(mask_json[3]["mask"]))
    # mask4 = torch.from_numpy(np.array(mask_json[4]["mask"]))
    bg_masks = torch.bitwise_not(torch.bitwise_or(mask1, torch.bitwise_or(mask2,mask3)))
    mask1 = transforms.Resize((64,64))(mask1.float()).view(1,1,64,64)
    mask2 = transforms.Resize((64,64))(mask2.float()).view(1,1,64,64)
    mask3 = transforms.Resize((64,64))(mask3.float()).view(1,1,64,64)
    # mask4 = transforms.Resize((64,64))(mask4.float()).view(1,1,64,64)
    bg_masks = transforms.Resize((64,64))(bg_masks.float()).view(1,1,64,64)
    plt.imshow(np.concatenate([mask1.numpy()[0,0], mask2.numpy()[0,0], mask3.numpy()[0,0], bg_masks.numpy()[0,0]], axis=1))
    plt.savefig("./pz_masks.png")
    masks = torch.cat([bg_masks, mask2, mask1, mask3], dim=0).numpy()
    ...
    for (x, y , w, h) in bboxs:
        cv2.rectangle(face_img, (x,y), (x+w, y+h), (255,255,0),1)
...
def assign_bbox_to_ids(model, model_config, args, **kwargs):
    controller = AttentionStore_wo_astar(args.low_resource)
    ...
    # mask_A = normalised_noun_map1.detach() > 0.025
    ...
    # mask_B = normalised_noun_map2.detach() > 0.025
    ...
    # id1_iou1 = torch.sum(mask_A[bboxs[0][0]:bboxs[0][2], bboxs[0][1]:bboxs[0][3]])
    # id1_iou2 = torch.sum(mask_A[bboxs[1][0]:bboxs[1][2], bboxs[1][1]:bboxs[1][3]])
    ...
    #     matched_bbox["id1"] = [bboxs[0]]
    #     matched_bbox["id2"] = [bboxs[1]]
    # else:
    #     matched_bbox["id1"] = [bboxs[1]]
    #     matched_bbox["id2"] = [bboxs[0]]
...
def generate_original_image(model, model_config, args, **kwargs):
    ...
    with torch.no_grad():
        masks = assign_bbox_to_ids(model, model_config, args, **kwargs)
        ...
        for i, step in enumerate(iterator):
            ...
            if image_for_ddim is not None:
                ...
                if(masks is not None):
                    ...
                    mask_prompt_list = [" photo of a beach", "a photo of sks person on the beach", "a photo of ks person on the beach", "a photo of ata person on the beach"]
                    if(multi_diff_ddim is not None):
                        md_face_img = multi_diff_ddim['face_img']
                        md_img_ori = multi_diff_ddim['image_ori']
                        num_of_person = md_img_ori['num_of_person']
                        if(md_img_ori.get("model2") is not None):
                            id1_dict = md_img_ori["id1_multi_diff"]
                            id2_dict = md_img_ori["id2_multi_diff"]
                            h_space_id1 = {'h_space_feat': None, 't':ts.repeat(2)}
            ...
            if(not do_multi_diff):
                ...
                if(use_prompt_mixing and i < steps_for_prompt_mixing):
                    ...
                    if(md_img_ori.get("model2") is None):
                        ...
                        # getting other context
                        if(orig_image_for_ddim is not None):
                            other_cond = self.model.get_learned_conditioning(mask_prompt, md_face_img, image_ori=md_img_ori,aligned_faces=aligned_faces,
                                                                                h_space=h_space)
                            other_cond = torch.cat([cond, other_cond])
                        ...
                        cond_list = []
                        cond_id1 = self.model.get_learned_conditioning(mask_prompt[:2], face_img=md_face_img, image_ori=id1_dict,
                                                                       aligned_faces=aligned_faces,h_space=h_space_id1)
                        # cond_id2 = md_img_ori.get("model2")[0].model.get_learned_conditioning(mask_prompt[2:], face_img=md_face_img, image_ori=id2_dict,
                        #                                                              aligned_faces=aligned_faces,h_space=h_space_id2)
                        cond_list.append(cond_id1)
                        for num in range(num_of_person-1)[::-1]:
                            if(num!=0):
                                cond_list.append(md_img_ori.get("model2")[num_of_person-2-num].model.get_learned_conditioning(mask_prompt[-1*num-1:-1*num], face_img=md_face_img, image_ori=id2_dict,
                                                                                        aligned_faces=aligned_faces,h_space=h_space_id2))
                        cond_list.append(md_img_ori.get("model2")[num_of_person-2].model.get_learned_conditioning(mask_prompt[-1:], face_img=md_face_img, image_ori=id2_dict,
                                                                                        aligned_faces=aligned_faces,h_space=h_space_id2))
                        cond = torch.cat(cond_list)
                        # cond = torch.cat([cond_id1, cond_id2])
                        # getting other context
                        if(orig_image_for_ddim is not None):
                            other_cond_list = []
                            other_cond1 = self.model.get_learned_conditioning(mask_prompt[:2], md_face_img, image_ori=id1_dict,aligned_faces=aligned_faces,
                                                                                h_space=h_space_id1)
                            other_cond_list.append(other_cond1)
                            for num in range(num_of_person-1)[::-1]:
                                if(num!=0):
                                    other_cond_list.append(md_img_ori.get("model2")[num_of_person-2-num].model.get_learned_conditioning(mask_prompt[-1*num-1:-1*num], md_face_img, image_ori=id2_dict,aligned_faces=aligned_faces,
                                                                                            h_space=h_space_id2))
                            # other_cond3 = md_img_ori.get("model2").model.get_learned_conditioning(mask_prompt[-2:-1], md_face_img, image_ori=id2_dict,aligned_faces=aligned_faces,
                            #                                                     h_space=h_space_id2)
                            # other_cond4 = md_img_ori.get("model2").model.get_learned_conditioning(mask_prompt[-1:], md_face_img, image_ori=id2_dict,aligned_faces=aligned_faces,
                            #                                                     h_space=h_space_id2)
                            other_cond_list.append(md_img_ori.get("model2")[num_of_person-2].model.get_learned_conditioning(mask_prompt[-1:], md_face_img, image_ori=id2_dict,aligned_faces=aligned_faces,
                                                                                            h_space=h_space_id2))
                            other_cond = torch.cat(other_cond_list)
                            # other_cond = torch.cat([other_cond1, other_cond2])
                            other_cond = torch.cat([cond, other_cond])

                        out_ids_list = []
                        outs_id1 = self.p_sample_ddim(args, latent_view[:2], cond[:2], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning, other_cond=other_cond1, orig_image_for_ddim=None)
                        out_ids_list.append(outs_id1)
                        for num in range(num_of_person-1)[::-1]:
                            if(num!=0):
                                outs_id2 = self.p_sample_ddim(args, latent_view[-1*(num+1):-1*num], cond[-1*(num+1):-1*num], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                        quantize_denoised=quantize_denoised, temperature=temperature,
                                                        noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                        corrector_kwargs=corrector_kwargs,
                                                        unconditional_guidance_scale=unconditional_guidance_scale,
                                                        unconditional_conditioning=unconditional_conditioning, other_cond=other_cond[-1*(num+1):-1*num], orig_image_for_ddim=None,
                                                        model2=md_img_ori.get("model2")[num_of_person-2-num].model)
                            out_ids_list.append(outs_id2)
                        out_ids_list.append(self.p_sample_ddim(args, latent_view[-1:], cond[-1:], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                        quantize_denoised=quantize_denoised, temperature=temperature,
                                                        noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                        corrector_kwargs=corrector_kwargs,
                                                        unconditional_guidance_scale=unconditional_guidance_scale,
                                                        unconditional_conditioning=unconditional_conditioning, other_cond=other_cond[-1:], orig_image_for_ddim=None,
                                                        model2=md_img_ori.get("model2")[num_of_person-2].model))
                        value_id1 = (outs_id1[0] * masks[:2]).sum(dim=0,keepdims=True)
                        count_id1 = masks[:2].sum(dim=0, keepdims=True)
                        other_value, value = [], value_id1
                        other_count, count = [], count_id1
                        for num in range(num_of_person-1):
                            if(num!=0):
                                other_value.append((out_ids_list[num][0] * masks[-1*(num+1):-1*num]).sum(dim=0,keepdims=True))
                                other_count.append(masks[-1*(num+1):-1*num].sum(dim=0, keepdims=True))
                            else:
                                other_value.append((out_ids_list[num][0] * masks[-1*(num+1):]).sum(dim=0,keepdims=True))
                                other_count.append(masks[-1*(num+1):].sum(dim=0, keepdims=True))
                        
                        for num in range(num_of_person-1):
                            value = value + other_value[num]
                            count = count + other_count[num]
    ...
    def p_sample_ddim(self, args, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            if(model2 is not None):
                e_t_uncond, e_t = model2.apply_model(x_in, t_in, c_in).chunk(2)
            else:
