...
def assign_bbox_to_ids(model, model_config, args, **kwargs):
    ...
    bboxs, masks = get_face_bounding_boxes(image[0][:,:,::-1])
                ...
                if(use_prompt_mixing and i < steps_for_prompt_mixing):
                    ...
                    if(md_img_ori.get("model2") is None):
                        ...
                        cond_id1 = self.model.get_learned_conditioning(mask_prompt[1:2], face_img=md_face_img, image_ori=id1_dict,
                                                                       aligned_faces=aligned_faces,h_space=h_space_id1)
                        cond_id2 = md_img_ori.get("model2")[1].model.get_learned_conditioning(mask_prompt[2:3], face_img=md_face_img, image_ori=id2_dict,
                                                                                     aligned_faces=aligned_faces,h_space=h_space_id2)
                        cond_id3 = md_img_ori.get("model2")[2].model.get_learned_conditioning(mask_prompt[3:], face_img=md_face_img, image_ori=id2_dict,
                                                                                     aligned_faces=aligned_faces,h_space=h_space_id2)
                        cond = torch.cat([cond_id0, cond_id1, cond_id2, cond_id3])
                        # getting other context
                        if(orig_image_for_ddim is not None):
                            other_cond0 = md_img_ori.get("model2")[0].model.get_learned_conditioning(mask_prompt[:1], md_face_img, image_ori=id1_dict,aligned_faces=aligned_faces,
                                                                                h_space=h_space_id1)
                            other_cond1 = self.model.get_learned_conditioning(mask_prompt[1:2], md_face_img, image_ori=id1_dict,aligned_faces=aligned_faces,
                                                                                h_space=h_space_id1)
                            other_cond2 = md_img_ori.get("model2")[1].model.get_learned_conditioning(mask_prompt[2:3], md_face_img, image_ori=id2_dict,aligned_faces=aligned_faces,
                                                                                h_space=h_space_id2)
                            other_cond3 = md_img_ori.get("model2")[2].model.get_learned_conditioning(mask_prompt[3:], md_face_img, image_ori=id2_dict,aligned_faces=aligned_faces,
                                                                                h_space=h_space_id2)
                            other_cond = torch.cat([other_cond0, other_cond1, other_cond2, other_cond3])
                            other_cond = torch.cat([cond, other_cond])
                        ...
                        outs_id2 = self.p_sample_ddim(args, latent_view[2:3], cond[2:3], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                        ...
                        outs_id3 = self.p_sample_ddim(args, latent_view[3:], cond[3:], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning, other_cond=other_cond3, orig_image_for_ddim=None,
                                                model2=md_img_ori.get("model2")[2],
                                                )   
                        ...
                        value_id2 = (outs_id2[0] * masks[2:3]).sum(dim=0,keepdims=True)
                        count_id2 = masks[2:3].sum(dim=0, keepdims=True)
                        value_id3 = (outs_id3[0] * masks[3:]).sum(dim=0,keepdims=True)
                        count_id3 = masks[3:].sum(dim=0, keepdims=True)
                        value = value_id0 + value_id1 + value_id2 + value_id3
                        count = count_id0 + count_id1 + count_id2 + count_id3 
