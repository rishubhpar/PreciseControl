...
def get_face_bounding_boxes(face_img):
    ...
    mask_json = json.load(open("./mask_4.json"))
    ...
    bg_masks = torch.bitwise_not(torch.bitwise_or(torch.bitwise_or(mask1, mask2), torch.bitwise_or(mask3, mask4)))
    ...
    plt.imshow(np.concatenate([mask1.numpy()[0,0], mask2.numpy()[0,0], mask3.numpy()[0,0], mask4.numpy()[0,0], bg_masks.numpy()[0,0]], axis=1))
    plt.savefig("./pz_masks.png")
    masks = torch.cat([bg_masks, mask3, mask1, mask4, mask2], dim=0).numpy()
        ...
        for i, step in enumerate(iterator):
            ...
            if image_for_ddim is not None:
                ...
                if(masks is not None):
                    ...
                    mask_prompt_list = [" photo of a mountain", "a photo of brad pitt taking a hike in the mountains", "a photo of taylor swift taking a hike in the mountains",
                                        "a photo of bill gates taking a hike in mountains", "a photo of ryan gosling taking a hike in the mountains"]
            ...
            if(not do_multi_diff):
                ...
                if(use_prompt_mixing and i < steps_for_prompt_mixing):
                    ...
                    if(md_img_ori.get("model2") is None):
                        ...
                        cond_id2 = md_img_ori.get("model2").model.get_learned_conditioning(mask_prompt[-3:-2], face_img=md_face_img, image_ori=id2_dict,
                                                                                     aligned_faces=aligned_faces,h_space=h_space_id2)
                        cond_id3 = md_img_ori.get("model2").model.get_learned_conditioning(mask_prompt[-2:-1], face_img=md_face_img, image_ori=id2_dict,
                                                                                     aligned_faces=aligned_faces,h_space=h_space_id2)
                        cond_id4 = md_img_ori.get("model2").model.get_learned_conditioning(mask_prompt[-1:], face_img=md_face_img, image_ori=id2_dict,
                                                                                     aligned_faces=aligned_faces,h_space=h_space_id2)
                        cond = torch.cat([cond_id1, cond_id2, cond_id3, cond_id4])
                        # getting other context
                        if(orig_image_for_ddim is not None):
                            other_cond1 = self.model.get_learned_conditioning(mask_prompt[:2], md_face_img, image_ori=id1_dict,aligned_faces=aligned_faces,
                                                                                h_space=h_space_id1)
                            other_cond2 = md_img_ori.get("model2").model.get_learned_conditioning(mask_prompt[-3:-2], md_face_img, image_ori=id2_dict,aligned_faces=aligned_faces,
                                                                                h_space=h_space_id2)
                            ...
                            other_cond4 = md_img_ori.get("model2").model.get_learned_conditioning(mask_prompt[-1:], md_face_img, image_ori=id2_dict,aligned_faces=aligned_faces,
                                                                                h_space=h_space_id2)
                            other_cond = torch.cat([other_cond1, other_cond2, other_cond3, other_cond4])
                            other_cond = torch.cat([cond, other_cond])
                        ...
                        outs_id2 = self.p_sample_ddim(args, latent_view[-3:-2], cond[-3:-2], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning, other_cond=other_cond2, orig_image_for_ddim=None,
                                                model2=md_img_ori.get("model2"),
                                                )
                        outs_id3 = self.p_sample_ddim(args, latent_view[-2:-1], cond[-2:-1], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning, other_cond=other_cond3, orig_image_for_ddim=None,
                                                model2=md_img_ori.get("model2"),
                                                )

                        outs_id4 = self.p_sample_ddim(args, latent_view[-1:], cond[-1:], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning, other_cond=other_cond4, orig_image_for_ddim=None,
                                                model2=md_img_ori.get("model2"),
                                                )
                        ...
                        value_id2 = (outs_id2[0] * masks[-3:-2]).sum(dim=0,keepdims=True)
                        count_id2 = masks[-3:-2].sum(dim=0, keepdims=True)
                        value_id3 = (outs_id3[0] * masks[-2:-1]).sum(dim=0,keepdims=True)
                        count_id3 = masks[-2:-1].sum(dim=0, keepdims=True)
                        value_id4 = (outs_id4[0] * masks[-1:]).sum(dim=0,keepdims=True)
                        count_id4 = masks[-1:].sum(dim=0, keepdims=True)
                        value = value_id1 + value_id2 + value_id3 + value_id4
                        count = count_id1 + count_id2 + count_id3 + count_id4
                        
                        # value = value_id1 + value_id2
                        # count = count_id1 + count_id2
                        img = torch.where(count > 0, value / count, value)
                # cond_list_for_each_timestep.append(cond.detach().cpu().numpy())
                # img = outs[0]
                # img = self.controller.step_callback(img)
