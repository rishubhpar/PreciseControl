...
from ldm.modules.prompt_mixing.attention_utils import show_cross_attention, aggregate_attention
...
def generate_original_image(model, model_config, args, **kwargs):
    ...
    return image, x_t, orig_all_latents, orig_mask, average_attention, controller, masks
...
class DDIMSamplerWrapper(object):
    ...
    def ddim_sampling(self, args, cond, shape,
        ...
        self.enbale_attn_controller_changes = True if (post_background or not do_multi_diff) else False
        # self.enbale_attn_controller_changes = True
        object_mask = None
        object_masks_list = orig_image_for_ddim.get("object_mask", None) if orig_image_for_ddim is not None else None
        object_masks_list = [] if object_masks_list is None else object_masks_list
        self.diff_step = 0
        all_latents = []
            ...
            if(masks is not None):
                ...
                if(multi_diff_ddim is not None):
                    ...
                    if(md_img_ori.get("model2") is not None):
                        ...
                        md_img_ori["model2"][1].diff_step = 0
        ...
        for i, step in enumerate(iterator):
            ...
            # if(orig_image_for_ddim is not None):
            #     img = torch.cat([img,img]) if img.shape[0] == 1 else img
            h_space = {'h_space_feat': None, 't':ts.repeat(1)}
            h_space_id1 = {'h_space_feat': None, 't':ts.repeat(1)}
            h_space_id2 = {'h_space_feat': None, 't':ts.repeat(1)}
            
            if(multi_diff_ddim is not None):
                md_img_ori["model2"][1].input_cross_index = 0
                md_img_ori["model2"][1].middle_cross_index = 0
                md_img_ori["model2"][1].output_cross_index = 0
            ...
            if(not do_multi_diff):
                ...
                latent_view = img[1].clone().repeat(num_of_prompts, 1, 1, 1)
                ...
                if(use_prompt_mixing and i < steps_for_prompt_mixing):
                    ...
                    if(md_img_ori.get("model2") is None):
                        ...
                        cond_id0 = md_img_ori.get("model2")[0].model.get_learned_conditioning(mask_prompt[:1], face_img=md_face_img, image_ori=id1_dict,
                                                                       aligned_faces=aligned_faces,h_space=h_space_id1)
                        cond_id1 = self.model.get_learned_conditioning(mask_prompt[1:2], face_img=md_face_img, image_ori=id1_dict,
                                                                       aligned_faces=aligned_faces,h_space=h_space_id1)
                        cond_id2 = md_img_ori.get("model2")[1].model.get_learned_conditioning(mask_prompt[2:], face_img=md_face_img, image_ori=id1_dict,
                        ...
                        if(orig_image_for_ddim is not None):
                            ...
                            # other_cond = torch.cat([cond, other_cond])
                            other_cond0 = torch.cat([cond_id0, other_cond0])
                            other_cond1 = torch.cat([cond_id1, other_cond1])
                            other_cond2 = torch.cat([cond_id2, other_cond2])

                        outs_id0 = self.p_sample_ddim(args, torch.cat([img[0].unsqueeze(0),latent_view[:1]]), cond[:1], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning, other_cond=other_cond0, orig_image_for_ddim=orig_image_for_ddim,
                                                model2=md_img_ori.get("model2")[0],)

                        self.input_cross_index = 0
                        self.middle_cross_index = 0
                        self.output_cross_index = 0
                        outs_id1 = self.p_sample_ddim(args, torch.cat([img[0].unsqueeze(0),latent_view[1:2]]), cond[1:2], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning, other_cond=other_cond1, orig_image_for_ddim=orig_image_for_ddim)
                        # self.enbale_attn_controller_changes = False
                        outs_id2 = md_img_ori.get("model2")[1].p_sample_ddim(args, torch.cat([img[0].unsqueeze(0),latent_view[2:]]), cond[2:], ts, index=index, use_original_steps=ddim_use_original_steps,
                                                quantize_denoised=quantize_denoised, temperature=temperature,
                                                noise_dropout=noise_dropout, score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs,
                                                unconditional_guidance_scale=unconditional_guidance_scale,
                                                unconditional_conditioning=unconditional_conditioning, other_cond=other_cond2, orig_image_for_ddim=orig_image_for_ddim,
                                                model2=md_img_ori.get("model2")[1],
                                                )
                        
                        # img1 = outs_id1[0]
                        # img2 = outs_id2[0]
                        # img1 = self.controller.step_callback(img1)
                        # img2 = md_img_ori.get("model2")[1].controller.step_callback(img2)
                        img0 = outs_id0[0]
                        img1 = outs_id1[0]
                        img2 = outs_id2[0]
                        ...
                        if post_background and (self.diff_step == args.background_blend_timestep):
                            if(len(object_masks_list)!=2):
                                object_mask1 = Segmentor(self.controller,
                                                        [mask_prompt_list[1]],
                                                        args.num_segments,
                                                        args.background_segment_threshold,
                                                        background_nouns=args.background_nouns)\
                                    .get_background_mask(mask_prompt_list[1].split(" ").index("sks")+1)
                                object_masks_list.append(object_mask1)
                            else:
                                print("prev mask available")
                                object_mask1 = object_masks_list[0]
                            self.enbale_attn_controller_changes = False
                            pm_mask = object_mask1.astype(np.bool8) + orig_mask.astype(np.bool8)
                            ...
                            img1 = pm_mask * img1 + (1 - pm_mask) * img1

                            
                            if(len(object_masks_list)!=2):
                                object_mask2 = Segmentor(md_img_ori.get("model2")[1].controller,
                                                        [mask_prompt_list[2]],
                                                        args.num_segments,
                                                        args.background_segment_threshold,
                                                        background_nouns=args.background_nouns)\
                                    .get_background_mask(mask_prompt_list[2].split(" ").index("ks")+1)
                                object_masks_list.append(object_mask2)
                            else:
                                print("prev mask available")
                                object_mask2 = object_masks_list[1]
                            md_img_ori.get("model2")[1].enbale_attn_controller_changes = False
                            pm_mask = object_mask2.astype(np.bool8) + orig_mask.astype(np.bool8)
                            pm_mask = torch.from_numpy(pm_mask).float().cuda()
                            shape = (1, 1, pm_mask.shape[0], pm_mask.shape[1])
                            pm_mask = torch.nn.Upsample(size=(64, 64), mode='nearest')(pm_mask.view(shape))
                            # plt.imshow(pm_mask.cpu().numpy()[0, 0])
                            ...
                            mask_eroded = dilate(pm_mask.cpu().numpy()[0, 0], np.ones((3, 3), np.uint8), iterations=1)
                            pm_mask = torch.from_numpy(mask_eroded).float().cuda().view(1, 1, 64, 64)
                            img2 = pm_mask * img2 + (1 - pm_mask) * img2

                        
                        # outs_id1[0] = outs_id1[0][1].unsqueeze(0)
                        # outs_id2[0] = outs_id2[0][1].unsqueeze(0)
                        # value_id0 = (img0 * masks[:1]).sum(dim=0,keepdims=True)
                        # count_id0 = masks[:1].sum(dim=0, keepdims=True)
                        # value_id1 = (img1 * masks[1:2]).sum(dim=0,keepdims=True)
                        # count_id1 = masks[1:2].sum(dim=0, keepdims=True)
                        # value_id2 = (img2 * masks[2:]).sum(dim=0,keepdims=True)
                        # count_id2 = masks[2:].sum(dim=0, keepdims=True)
                        value_id0 = (img0 * masks[:1])
                        count_id0 = masks[:1]
                        value_id1 = (img1 * masks[1:2])
                        count_id1 = masks[1:2]
                        value_id2 = (img2 * masks[2:])
                        count_id2 = masks[2:]
            ...
            if post_background and (self.diff_step == args.background_blend_timestep) and not do_multi_diff:
            ...
            self.diff_step += 1
            if(multi_diff_ddim is not None):
                md_img_ori["model2"][1].diff_step += 1
            ...
            if(do_multi_diff and i > steps_for_prompt_mixing):
                del cond_id0, cond_id1, cond_id2, other_cond0, other_cond1, other_cond2, outs_id0, outs_id1, outs_id2
        ...
        return image, None, all_latents, object_masks_list
        ...
        def ca_forward(model_self, place_in_unet):
            ...
            def forward(x, context=None, mask=None):
                ...
                if self.enbale_attn_controller_changes:
                    # print("model id :", self.controller.model_id)
    ...
    def p_sample_ddim(self, args, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            if(model2 is not None):
                ...
                # print("current memory used 4:", torch.cuda.memory_allocated())
                e_t_uncond, e_t = self.model.apply_model(x_in, t_in, c_in).chunk(2)
                # print("current memory used 4.1:", torch.cuda.memory_allocated())
            # print("current memory used 3.1:", torch.cuda.memory_allocated())
