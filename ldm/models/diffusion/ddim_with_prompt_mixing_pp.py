...
from ldm.modules.prompt_mixing.prompt_to_prompt_controllers_pp import DummyController, AttentionStore
...
def get_bbox(dets, shape_ratio, cv2=False):
    if(not cv2):
        ...
        dets = dets["bbox"]
        tl_y = max(0, int(dets[0] * shape_ratio) - 1)
        tl_x = max(0, int(dets[1] * shape_ratio) - 1)
        h = min(int((dets[2] - dets[0]) * shape_ratio) + 2, 32)
        w = min(int((dets[3] - dets[1]) * shape_ratio) + 2, 32)
    return (tl_x, tl_y, w, h)

def get_face_bounding_boxes(face_img):
    shape_ratio = 32 / face_img.shape[0]
    # detector = cv2.CascadeClassifier('./weights/haarcascade_frontalface_default.xml') 
    # gray = cv2.cvtColor(face_img, cv2.COLOR_BGR2GRAY)
    # dets = detector.detectMultiScale(gray, 1.3, 5) 

    detector = Ultralight320Detector()
    dets = detector.detect_faces(face_img)
    print("detected faces: ", dets)

    bboxs = [get_bbox(det, shape_ratio, True) for det in dets]
    # bboxs = [(2, 1, 15, 15)]
...
def assign_bbox_to_ids(model, model_config, args, two_ids = False, **kwargs):
    ...
    import torchvision
    from torchvision import transforms
    torchvision.utils.save_image(transforms.ToTensor()(image[0]), "./pmm_sample_img_orig.jpg")

    print("image ", image.shape, np.max(image), np.min(image))
    bboxs = get_face_bounding_boxes(image[0][:,:,::-1].astype(np.uint8))
    ...
    print("max and min value of attn map : ", torch.max(normalised_noun_map1), torch.min(normalised_noun_map1))
    mask_A = normalised_noun_map1.detach() > 0.03
    # for burger cross attn box
    # two_burger = True
    # if(len(bboxs)==0):
    #     bboxs = cv2.boundingRect(mask_A.cpu().numpy().astype(np.uint8))
    #     if(two_burger):
    #         token_attn = attn[:,:,4]
    #         curr_noun_map = token_attn.repeat_interleave(2, dim=0).repeat_interleave(2, dim=1)
    #         normalised_noun_map2 = torch.nn.functional.normalize(curr_noun_map, p=2, dim=(0,1))
    #         mask_B = normalised_noun_map2.detach() > 0.03
    #         bboxs = [bboxs, cv2.boundingRect(mask_B.cpu().numpy().astype(np.uint8))]
    #     else:
    #         bboxs = [bboxs]
    # bboxs = [[6,15,8,8], [20, 10, 10, 8]]

    # plt.imshow(mask_A.detach().cpu().numpy())
    # plt.savefig(os.path.join("pz_burger_cross_attn1_{}.png".format(str(50).zfill(2))))

    # if(two_burger):
    #     plt.imshow(mask_B.detach().cpu().numpy())
    #     plt.savefig(os.path.join("pz_burger_cross_attn2_{}.png".format(str(50).zfill(2))))
    ...
    # matching bbox with cross attn map
        # correct the matching algorithm now hardcoding
        id1_iou1 = torch.sum(mask_A[bboxs[0][0]:bboxs[0][2], bboxs[0][1]:bboxs[0][3]]) / ((bboxs[0][2] - bboxs[0][0]) * (bboxs[0][3] - bboxs[0][1]))
        id1_iou2 = torch.sum(mask_A[bboxs[1][0]:bboxs[1][2], bboxs[1][1]:bboxs[1][3]]) / ((bboxs[1][2] - bboxs[1][0]) * (bboxs[1][3] - bboxs[1][1]))

        # mask_A[bboxs[0][0]:bboxs[0][2], bboxs[0][1]:bboxs[0][3]] = 0
        # plt.imshow(mask_A)
        # plt.savefig(os.path.join("pz_maska1_cropped_{}.png".format(str(50).zfill(2))))
        # mask_A[bboxs[1][0]:bboxs[1][2], bboxs[1][1]:bboxs[1][3]] = 0
        # plt.imshow(mask_A)
        # plt.savefig(os.path.join("pz_maska2_cropped_{}.png".format(str(50).zfill(2))))
        ...
        final_maskA = np.fliplr(np.rot90(final_maskA.detach().cpu().numpy(),3))
        final_maskB = np.fliplr(np.rot90(final_maskB.detach().cpu().numpy(),3))
        # for checking cross attns control
        # final_maskA = np.roll(final_maskA, 10, axis=0)
        # final_maskB = np.roll(final_maskB, 10, axis=0)
        loss_kernal_A = None
        loss_kernal_B = None
        plt.imshow(final_maskA)
        plt.savefig(os.path.join("pz_maska_{}.png".format(str(50).zfill(2))))
        plt.imshow(final_maskB)
    ...
    return final_maskA, final_maskB, loss_kernal_A, loss_kernal_B



def generate_original_image(model, model_config, args, **kwargs):
    with torch.no_grad():
        maskA, maskB, loss_kernal_A, loss_kernal_B = assign_bbox_to_ids(model, model_config, args,two_ids=kwargs["image_for_ddim"]["two_ids"], **kwargs)
        print("seed value ", kwargs["image_for_ddim"]["seed"])
    seed_everything(kwargs["image_for_ddim"]["seed"])
    masks = {"maska":maskA, "maskb":maskB, "loss_kernal_a":loss_kernal_A, "loss_kernal_b":loss_kernal_B}
    ...
    orig_mask = Segmentor(controller, kwargs["image_for_ddim"]['caption'], args.num_segments, args.background_segment_threshold, 
                        #   background_nouns=args.background_nouns).get_background_mask(kwargs["image_for_ddim"]["caption"][-1].split(" ").index("sks")+1)
                          background_nouns=args.background_nouns).get_background_mask(2)
    average_attention = controller.get_average_attention()
    # image, x_t, orig_all_latents, orig_mask, average_attention, controller = None, None, None, None, None, None
...
class DDIMSamplerWrapper(object):
    ...
    def ddim_sampling(self, args, cond, shape,
        ...
        if x_T is None:
            ...
            # np.random.seed(image_for_ddim["seed"])
            # img = torch.from_numpy(np.random.normal(0, 1, (4,shape[1],shape[2],shape[3]))).float().to(device)
        ...
        for i, step in enumerate(iterator):
            ...
            # getting other context
            if(orig_image_for_ddim is not None):
                if(i >= orig_image_for_ddim["steps_for_prompt_mixing"]):
                    c = orig_image_for_ddim["caption"]
                other_cond = self.model.get_learned_conditioning(c, face_img, image_ori=orig_image_for_ddim['image_ori'],aligned_faces=aligned_faces,
                                                                    h_space=h_space)
            ...
            save_dir = "./cross_attn_at_each_timestep/weights_0/"
            import os
            from torchvision import transforms
            import matplotlib.pyplot as plt
            ...
            plt.savefig(os.path.join(save_dir, "cross_attn1_{}.png".format(str(i).zfill(2))))
            if(two_ids):
                save_dir = "./cross_attn_at_each_timestep/weights_0/"
                import os
                from torchvision import transforms
                ...
                plt.savefig(os.path.join(save_dir, "cross_attn2_{}.png".format(str(i).zfill(2))))
