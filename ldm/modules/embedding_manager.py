...
def get_clip_token_for_string(tokenizer, string):
    ...
    # return tokens[0, 1]
    # print("tokentized string is ", tokenizer(string))
    return tokenizer(string)[0,1]
...
def get_embedding_for_clip_token(embedder, token):
    # print("embedding shape :", embedder(token.unsqueeze(0)).shape)
...
class EmbeddingManagerId(nn.Module):
    def __init__(
            ...
            regularization_weight: float = 1e-7,
            use_norm_reg: bool = False,
            use_aligned_faces: bool = False,
            use_hspace: bool = False,
            use_stylegan_based_mapper: bool = False,
            use_timestep_embedder: bool = False,
        ...
        self.use_aligned_faces = use_aligned_faces
        self.use_hspace = use_hspace
        ...
        if(True):
            ...
            # get_token_for_string = partial(get_clip_token_for_string, embedder.tokenizer)
            # get_embedding_for_tkn = partial(get_embedding_for_clip_token, embedder.transformer.text_model.embeddings)
            # celeb_basis = embedder.celeb_embeddings
            # token_dim = 768
            self.is_clip = True
            get_token_for_string = partial(get_clip_token_for_string, embedder.tokenizer)
        ...
        # For regularising text embedding output to be close to ["human"] clip embedding
        if(self.use_regularization or self.use_basis_offset):
            self.words_close_to_output_distribution = [str(domain_name)]
            self.token_for_output_distribution = [get_token_for_string(word) for word in self.words_close_to_output_distribution]
            self.clip_embedding_for_output_distribution = [get_embedding_for_tkn(token) for token in self.token_for_output_distribution]
            self.avg_clip_embedding_for_output_distribution = torch.mean(torch.stack(self.clip_embedding_for_output_distribution), dim=0)
        else:
            self.avg_clip_embedding_for_output_distribution = None

        if(use_norm_reg):
            self.reg_token = get_token_for_string(str(domain_name))
            self.reg_embedding = get_embedding_for_tkn(self.reg_token)
        ...
        self.meta_id_net = MetaIdNet(use_expert=False,
                                     ...
                                     domain_clip_embed=self.avg_clip_embedding_for_output_distribution,
                                     norm_reg_embedding=self.norm_reg_embedding,
                                     use_stylegan_based_mapper=use_stylegan_based_mapper,
                                     use_hspace=use_hspace,
                                     use_timestep_embedder=use_timestep_embedder,
                                     use_celeb_basis=use_celeb_basis,
                                     use_basis_offset=use_basis_offset,
                                     celeb_basis=celeb_basis,
        ...
        if img_ori is not None:
            ...
            for b_idx in range(b):
                if num_ids[b_idx] == 2:  # two persons in an image
                    ...
                    # print('[replaced]:', embedded_text[b_idx].max(dim=-1)[0])_momentum_update
    ...
    def forward(
        ...
        # if(img_ori is not None):
        #     print("num_ids is ", img_ori["num_ids"])
        self.celeb_embeddings = celeb_embeddings.to(device)
        if(not self.use_aligned_faces):
            aligned_faces = None
        ...
        if img_ori is not None:
            ...
            ids = img_ori["ids"]  # (N,(1+diff+1+diff))
            num_ids = img_ori["num_ids"]  # (N,)
            w_latents = img_ori.get("w_latents", None)
            delta_w = img_ori.get("delta_w", None)
            _, id_cnt = ids.shape
            if self.training:
                # print("In embedding manager, aligned images is ", aligned_faces)
                meta, cls, cef, reg_loss = self.meta_id_net.forward_multi_faces(faces, ids, celeb_embeddings, aligned_faces, h_space, w_latents)
                # meta:id_cnt*(N,es,768), cef:id_cnt*(N,es,h,inner_dim)
                meta1 = meta[0]  # x:(N,es*h,768), the main id
            ...
            elif(faces is not None and self.test_mode == 'image'):
                meta, cls, cef, reg_loss = self.meta_id_net.forward_multi_faces(faces, ids, celeb_embeddings,aligned_faces, h_space, w_latents, delta_w)
                ...
                cef1 = torch.zeros(b)
                cef2 = torch.zeros(b)
                cef3 = torch.zeros(b) # in training, the max #id is 2
            ...
            for b_idx in range(b):
                if num_ids[b_idx] == 2:  # two persons in an image
                ...
                elif num_ids[b_idx] == 4:
                    one_memo_l = self._momentum_update(meta1[b_idx], cef1[b_idx], ids[b_idx][0],interpolate_ids)
                    one_memo_r = self._momentum_update(meta2[b_idx], cef2[b_idx], ids[b_idx][1],interpolate_ids)
                    one_memo_3 = self._momentum_update(meta3[b_idx], cef3[b_idx], ids[b_idx][2],interpolate_ids)
                    one_memo_4 = self._momentum_update(meta4[b_idx], cef4[b_idx], ids[b_idx][3],interpolate_ids)

                    placeholder_token_l = self.string_to_token_dict[self.placeholder_strings[0]]
                    placeholder_token_r = self.string_to_token_dict[self.placeholder_strings[1]]
                    placeholder_token_3 = self.string_to_token_dict[self.placeholder_strings[2]]
                    placeholder_token_4 = self.string_to_token_dict[self.placeholder_strings[3]]
                    ...
                    placeholder_final_pos_r = placeholder_final_pos_lr[1]
                    placeholder_final_pos_3 = placeholder_final_pos_lr[2]
                    placeholder_final_pos_4 = placeholder_final_pos_lr[3]
                    for one_pos in placeholder_final_pos_l:
                        embedded_text[b_idx][one_pos] = one_memo_l.to(device)
                    for one_pos in placeholder_final_pos_r:
                        embedded_text[b_idx][one_pos] = one_memo_r.to(device)
                    for one_pos in placeholder_final_pos_3:
                        embedded_text[b_idx][one_pos] = one_memo_3.to(device)
                    for one_pos in placeholder_final_pos_4:
                        embedded_text[b_idx][one_pos] = one_memo_4.to(device)

                    # print('[replaced]:', embedded_text[b_idx].max(dim=-1)[0])
        return embedded_text
    
    
    # def interpolate(self, id1, id2, num_of_interpolation=5, ratio=1):
    #     if(len(self.id_embeddings) <= 1):
    #         print("only one id embedding, cannot interpolate")
    #     else:
    #         total_ids = len(self.id_embeddings)
    #         self.interpolated_id_embeddings = [self.id_embeddings[id1]]
    #         embedding_diff = self.id_embeddings[id2] - self.id_embeddings[id1]
    #         for i in range(num_of_interpolation):
    #             self.interpolated_id_embeddings.append(self.id_embeddings[id1] + embedding_diff * (i+1) / num_of_interpolation)

    #         return self.interpolated_id_embeddings
    ...
    def _momentum_update(self, one_pred_embedding: torch.Tensor,
        ...
        if not self.training:
            if self.test_mode == 'coefficient':
                ...
                print("numerber of identity : ",len(self.id_coefficients))
                if(sum(interpolate_ids)!=0):
                    print("Doing interpolation between id ",interpolate_ids[0]," and ",interpolate_ids[1], " with ratio ",interpolate_ids[2]/interpolate_ids[3] )
                    x1 = self.id_coefficients[interpolate_ids[0]]
                    x2 = self.id_coefficients[interpolate_ids[1]]
                    x = x1 + (x2-x1)*interpolate_ids[2]/interpolate_ids[3]
                else:
    ...
    def _calc_id_neg_loss(self, meta, cls, ids, cef, reg_loss, device):
        if meta is None:
            return
        ...
        if(self.use_regularization or self.use_basis_offset):
            lambda_output_dist = self.regularization_weight   # previous value was 1e-7
            mapper_output_reg = reg_loss
        else:
            lambda_output_dist = 0
            mapper_output_reg = 0
        ...
        self.id_neg_loss = loss_cosine * 0 + loss_cls * 0 + loss_reg * 1 + loss_contra * 1 + mapper_output_reg * lambda_output_dist
