...
class FrozenOpenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, arch="ViT-H-14", version="laion2b_s32b_b79k", device="cuda", max_length=77,
                 ...
                 use_domain_regularization=False,
                 domain_regularization_weight=1e-5,
        ...
        # self.transformer = model.token_embedding
        ...
        if use_celeb:
            ...
            self._get_clip_face_basis_embeddings(face_basis_path)
            self.n_components = n_components
        
        if use_clip_face_basis:
            self._get_clip_face_basis_embeddings(face_basis_path)

        self.use_domain_regularization = use_domain_regularization
        if self.use_domain_regularization:
            self.domain_clip_embedding = None
            self.domain_loss = 0
    ...
    def encode_with_transformer(self, text, 
        ...
        # do token swapping
        x = embedding_manager(text, x, face_img, image_ori,
                                                      self.celeb_embeddings, aligned_faces, h_space,  interpolate_ids)

        x = x + self.model.positional_embedding
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.text_transformer_forward(x, attn_mask=self.model.attn_mask)
        x = x.permute(1, 0, 2)  # LND -> NLD
        #print norm of each token in the sentence
        # print(torch.norm(x, dim=-1))
        # print(torch.argmax(torch.norm(x, dim=-1), dim=1), torch.norm(x, dim=-1).shape)
        if(only_embedding):
            return x
        if(self.use_domain_regularization):
            if(self.domain_clip_embedding is None):
                self.domain_clip_embedding = self.forward(["a photo of a face of person person person"], embedding_manager=embedding_manager, 
                                                          only_embedding=True, face_img=face_img, image_ori=image_ori, celeb_embeddings=celeb_embeddings,
                                                            aligned_faces=aligned_faces, h_space=h_space, interpolate_ids=interpolate_ids, **kwargs)
                self.domain_clip_embedding[:, 0:7, :] = 0
                self.domain_clip_embedding[:, 9:, :] = 0

            domain_clip_embedding = self.domain_clip_embedding.repeat(x.shape[0], 1, 1)
            self.domain_loss = torch.norm(x[:, 7:9, :], dim=-1).mean()
            x = domain_clip_embedding.to(x.device) + 0.1 * x
    ...
    def domain_regularizer_loss(self):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cuda", max_length=77,
        ...
        def embedding_forward(
                ...
                if embedding_manager is not None and not two_celebs and not multi_celebs and not word_interpolation:
                    ...
                    # print("Its running")
                    # print("Its running and align faces is ", aligned_faces)
                    inputs_embeds = embedding_manager(input_ids, inputs_embeds, face_img, image_ori,
                                                      celeb_embeddings, aligned_faces, h_space,  interpolate_ids)
        ...
        def text_encoder_forward(
            ...
            hidden_states = self.embeddings(input_ids=input_ids, position_ids=position_ids,
                                            ...
                                            h_space=h_space,
                                            interpolate_ids=interpolate_ids,
                                            )
            if only_embedding:
                return hidden_states
