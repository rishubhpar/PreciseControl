...
class MetaIdNet(nn.Module):
    def __init__(self,
        ...
        self.id_model = None
        self.load_fr_net()
        self.load_e4e_with_decoder('/mnt/data/rishubh/sachi/CelebBasisV2/weights/encoder/e4e_ffhq_encode.pt')
        ...
        self.stylegan_mlp = StyleVectorizer(18 * 512, inner_dim * self.num_es * self.heads,
    ...
    def _celebs_forward_old(self, img: torch.Tensor, id_idx: torch.Tensor,
                        celebs_embeds: torch.Tensor, aligned_faces=None):
        if not self.use_rm_mlp:
            with torch.no_grad():
                ...
                # print("[w_latent] ", w_latents.shape)
                # latent_img, _= self.e4e_encoder.decoder([w_latents[0].unsqueeze(0)], input_is_latent=True, randomize_noise=False, return_latents=True)
                # print("[latent_img]", latent_img.shape)
                w_latents = w_latents.reshape(w_latents.shape[0], -1)

            x = self.stylegan_mlp(w_latents)  # x:(N,num_es*heads*inner_dim)
    ...
    def _celebs_forward(self, img: torch.Tensor, id_idx: torch.Tensor,
        ...
        if not self.use_rm_mlp:
            with torch.no_grad():
                ...
                # print("[latent_img]", latent_img.shape)
                w_latents = w_latents.reshape(w_latents.shape[0], -1)
                # print("[w_latent_new]", w_latents.shape)
    ...
    def forward(self, img, id_idx, celebs_embeds=None, aligned_faces=None):
        if self.use_celebs:
            return self._celebs_forward(img, id_idx, celebs_embeds, aligned_faces)
        else:
    ...
    def forward_multi_faces(self, img_multi: torch.Tensor, id_multi: torch.Tensor, celeb_embeds=None, aligned_faces=None):
        ...
        # print("[b, num, id_multi, chunked, concatenated]", b, num, id_multi.shape, len(chunked), concatenated.shap
        vec, cls, cef = self.forward(concatenated,
                                     torch.flatten(id_multi),
                                     celeb_embeds, align_concatenated)
