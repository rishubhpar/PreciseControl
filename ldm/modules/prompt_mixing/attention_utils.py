...
from ldm.modules.prompt_mixing.prompt_to_prompt_controllers import AttentionStore
...
def get_current_cross_attn(attention_store: AttentionStore, res: int, from_where: List[str], is_cross: bool, select: int, prompts):
    out = []
    attention_maps = attention_store.get_current_cross_attention()
    num_pixels = res ** 2
    for location in from_where:
        for item in attention_maps[f"{location}_{'cross' if is_cross else 'self'}"]:
            if item.shape[1] == num_pixels:
