...
from src.lora.lora_diffusion.lora import inject_trainable_lora, monkeypatch_or_replace_lora

# from huggingface_hub import login
# access_token_read = "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"
...
def load_model_from_config(config, ckpt, embedding_path, verbose=False):
    ...
    if(config.model.params.use_lora_finetuning == True):
...
def main():
    ...
    model = load_model_from_config(config, f"{opt.ckpt}", opt.embedding_path)
    ...
    if not opt.from_file:
        ...
        temp_pos, temp_neg = "{}", "magzine, frame, tiled, repeated, multiple people, multiple faces, group of people, split frame, multiple panel, \
            split image, watermark, boarder, diptych, triptych, black and white, low quality, blurry image, out of frame, cropped"
        # temp_pos, temp_neg = "{}", ""
    ...
    if config.model.params.personalization_config.params.test_mode == "image":
        ...
        if opt.eval_dataset in ('vgg0', 'vgg1'):
            ...
            test_id2 = 0
            id1 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/stylegan3/edited/{}".format(
                os.listdir("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/stylegan3/edited/")[opt.eval_id1])).convert("RGB")
            id2 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/stylegan3/edited/{}".format(
                os.listdir("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/stylegan3/edited/")[test_id2])).convert("RGB")
            # id1 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/comparision/edited/{}".format(
            #     os.listdir("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/comparision/edited/")[opt.eval_id1])).convert("RGB")
            # id2 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/comparision/edited/{}".format(
            #     os.listdir("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/comparision/edited/")[test_id2])).convert("RGB")
    ...
    import json
    delta_w_dict = json.load(open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/flame_delta_w_dict.json"))
    # delta_w_dict = json.load(open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/latent_directions_flame.json"))
    ...
    grid_count = len(os.listdir(outpath)) - 1 if len(os.listdir(outpath)) != 0 else 0

    img_folder_name = "comp"
    files_list = sorted(os.listdir("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/".format(img_folder_name)))
    # print(files_list[:100])
    # files_list = ["1.png", "3_0.png", "5.png", "5_0.png", "7.png"]

    for l, img in enumerate(files_list):
        if(img.endswith(".jpg") or img.endswith(".png") or img.endswith(".jpeg")):
            img = '1.jpg'
            id1 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/{}".format(img_folder_name, img)).convert("RGB")
            faces = pil_to_4d(id1)

            if(two_ids):
                id2 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/{}".format("attr_test", "7.png")).convert("RGB")
                face2 = pil_to_4d(id2)
        else:
            continue
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        ...
                        for prompts in tqdm(data, desc="data"):
                            if(opt.interpolate_ids[2] != -1):
                                ...
                                prompt_mixing_prompt = [prompt_mix.replace("ks person","brad pitt") for prompt_mix in prompt_mixing_prompt]
                                ...
                                if not opt.skip_grid:
                                    ...
                                    n_rows = 5 if not two_ids else 6
                                    # new_all_samples = []
                                    ...
                                    all_samples = list()
                                    del grid, one_grid, x_samples_ddim, samples_ddim
                                prompts_idx += 1
