...
from ldm.models.diffusion.ddim_with_multi_diff import DDIMSamplerWrapper, generate_original_image, assign_bbox_to_ids
...
def main():
    ...
    if not opt.from_file:
        ...
        temp_pos, temp_neg = "{}", "3D, deformed, blurry, bad atonomy, disfigured, distorted, deformed, bad art, boring, low quality, poorly rendered, blurry, out of focus\
            (((out of frame))), low resolution, distored face, cartoon, duplicate, multiple faces, bad eyes, bad face, cropped head, black and white, \
                plain background, low details, distorted detail, jpeg artifacts, \
    ...
    for i, files in enumerate(files_list):
        ...
        image_multi_diff = {
                ...
                "ids": (torch.tensor([i, 7], device=device)).unsqueeze(0).repeat(num_of_segments, 1),
                        ...
                        for prompts in tqdm(data, desc="data"):
                            if(opt.interpolate_ids[2] != -1):
                                ...
                                if(loop_through_weights):
                                    ...
                                    for weight_lambda in np.arange(0.2, 1.2, 0.1):
