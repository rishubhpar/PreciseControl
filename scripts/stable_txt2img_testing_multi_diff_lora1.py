...
from ldm.models.diffusion.ddim_with_multi_diff_lora import DDIMSamplerWrapper, generate_original_image, assign_bbox_to_ids
...
from ldm.modules.prompt_mixing.prompt_to_prompt_controllers_astar import DummyController, AttentionReplace, AttentionStore
...
def main():
    ...
    model = load_model_from_config(config, f"{opt.ckpt}", opt.embedding_path, opt.lora_finetuned, 0.1)
    ...
    class PMConfig:
        ...
        end_prompt_range: int = 0                                          # Number of steps to finish prompt-mixing
        ...
        end_preserved_obj_self_attn_masking: int = 0

        # real image
        real_image_path: str = ""

        # controllable background preservation
        background_post_process: bool = False
    ...
    if config.model.params.personalization_config.params.test_mode == "image":
        ...
        sample_id = 1
        two_ids = True
        multi_face = ["bengio.jpg"]
        num_of_person = len(multi_face) + 1 if two_ids else 1
        mask_prompt_list = ["a photo of a restuarant", "a sks person having dinner together", 
                            "a ks person having dinner together"]
        # mask_prompt_list = ["a photo of a nice restuarant", "sks person having dinner together", 
        #                     "ks person having dinner together", "ata person having dinner together"]
    ...
    for i, files in enumerate(files_list):
        ...
        other_models = []
        if(len(opt.lora2_path)!=0):
            model0 = DDIMSamplerWrapper(model, model_config=get_stable_diffusion_config(args))
            model2 = DDIMSamplerWrapper(model2, model_config=get_stable_diffusion_config(args))
            other_models.append(model0)
            other_models.append(model2)
        if(len(opt.lora3_path)!=0):
            model3 = DDIMSamplerWrapper(model3, model_config=get_stable_diffusion_config(args))
            other_models.append(model3)
        # model0 = DDIMSamplerWrapper(model, model_config=get_stable_diffusion_config(args))
        # other_models = [model0, model0]
        if(two_ids):
            if(attr is not None and attr != 'pose'):
                delta_w1 = torch.tensor(delta_w_dict[attr], device=device).repeat(1, 1, 1) * delta_w1_lambda
            else:
                delta_w1 = torch.zeros_like(torch.tensor(delta_w_dict['smile'], device=device).repeat(batch_size, 1, 1))
            if(attr2 is not None):
                delta_w2 = torch.tensor(delta_w_dict[attr2], device=device).repeat(1, 1, 1) * delta_w2_lambda
            else:
        ...
        if(not loop_through_weights):
            image_ori = {
                ...
                "delta_w": torch.tensor(delta_w_dict[attr], device=device).repeat(batch_size, 1, 1) * delta_w1_lambda,
            }
        ...
        image_multi_diff_id2 = {
                ...
                "num_ids": (torch.ones(2, dtype=torch.long).to(device)) * num_of_person,
                "delta_w": torch.cat([delta_w1, delta_w2], dim=0) if attr2 is not None else None,
        }
                        ...
                        for prompts in tqdm(data, desc="data"):
                            if(opt.interpolate_ids[2] != -1):
                                ...
                                if(loop_through_weights):
                                    ...
                                    if(opt.lora_finetuned):
                                        tune_lora_scale(model.model, opt.lora_scale)
                                    for weight_lambda in np.arange(0.1, 0.2, 0.1):
                                        
                                        if(two_ids):
                                            if(attr is not None and attr != 'pose'):
                                                delta_w1 = torch.tensor(delta_w_dict[attr], device=device).repeat(batch_size, 1, 1) * delta_w1_lambda
                                            else:
                                                delta_w1 = torch.zeros_like(torch.tensor(delta_w_dict['smile'], device=device).repeat(batch_size, 1, 1))
                                            if(attr2 is not None):
                                                delta_w2 = torch.tensor(delta_w_dict[attr2], device=device).repeat(batch_size, 1, 1) * delta_w2_lambda
                                            else:
                                                delta_w2 = torch.zeros_like(delta_w1)
                                        ...
                                        # save samples
                                        torchvision.utils.save_image(transforms.ToTensor()(image[0]), os.path.join(outpath_new, f'{grid_count:04}-{prompt_all[prompts_idx].replace(" ", "-")}_'
                                                                                f'{opt.img_suffix[:40]}_{0}_orig.jpg'))
