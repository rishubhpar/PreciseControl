...
def load_model_from_config(config, ckpt, embedding_path, use_lora_finetuned, verbose=False):
    ...
    if((config.model.params.use_lora_finetuning == True) and use_lora_finetuned):
        ...
        # tune_lora_scale(model.model, 1.0)
...
def main():
    ...
    model = load_model_from_config(config, f"{opt.ckpt}", opt.embedding_path, opt.lora_finetuned)
    ...
    if config.model.params.personalization_config.params.test_mode == "image":
        ...
        if opt.eval_dataset in ('vgg0', 'vgg1'):
            ...
            test_id2 = 0
            id1 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/attr_test/edited/{}".format(
                os.listdir("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/attr_test/edited/")[opt.eval_id1])).convert("RGB")
            id2 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/attr_test/edited/{}".format(
                os.listdir("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/attr_test/edited/")[test_id2])).convert("RGB")
    ...
    folder_name = "comp"
    files_list = os.listdir("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/".format(folder_name))
    ...
    for i, files in enumerate(files_list):
        files = "1.jpg"
        id1 = Image.open("/media/tets/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/{}".format(folder_name,files)).convert("RGB")
        faces = pil_to_4d(id1)
                        ...
                        for prompts in tqdm(data, desc="data"):
                            if(opt.interpolate_ids[2] != -1):
                                ...
                                if(loop_through_weights):
                                    ...
                                    image, x_T, all_latents, orig_mask, average_attention = generate_original_image(model, 
                                    ...
                                    for weight_lambda in np.arange(0.6, 1.8, 0.1):
                                        
                                        context_image_ori = {
                                                    "faces": faces,
                                                    "ids": (torch.tensor([i], device=device)).unsqueeze(0).repeat(batch_size, 1),
                                                    "num_ids": (torch.ones(batch_size, dtype=torch.long).to(device)),
                                                    "delta_w": torch.tensor(delta_w_dict[attr], device=device).repeat(batch_size, 1, 1) * weight_lambda,
                                                }
                                        seed_everything(opt.seed)
                                        prompt_mixing_prompt = [prompt.replace("sks person","brad pitt") for prompt in prompts]
                                        print("prompt_mixing_prompt", prompt_mixing_prompt)
