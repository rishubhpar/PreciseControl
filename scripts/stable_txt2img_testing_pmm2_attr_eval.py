...
def main():
    ...
    if config.model.params.personalization_config.params.test_mode == "image":
        ...
        # beard - 1.8, age70 - 1.6, smile - 1.8, black -1.4, yellow - 1.5, eyeglasses -1.8

        delta_w_lambda = 1
        attr = 'eyeglasses'
        attr2 = None
        attr1_range = np.round(np.arange(1.0, 2.0, 0.1), 3)
        attr2_range = np.round(np.arange(0.4, 1.8, 0.1), 3) if attr2 is not None else []
        save_folder = "bengio"
        ...
        if(os.path.exists(os.path.join(outpath,attr))):
            print("creating outpath")
            # shutil.rmtree(os.path.join(outpath,"original"))
            shutil.rmtree(os.path.join(outpath,attr))
        os.makedirs(os.path.join(outpath, attr), exist_ok=True)
    ...
    for i, files in enumerate(files_list):
        ...
        if(two_ids):
            id2 = Image.open("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/{}".format("attr_test", "7.png")).convert("RGB")
            face2 = pil_to_4d(id2)

        grid_count = len(os.listdir(outpath)) - 1
        # outpath_new = os.path.join(outpath, f"sample_{grid_count}")
        outpath_new = outpath
        os.makedirs(os.path.join(outpath_new,"original"), exist_ok=True)
                        ...
                        for prompts in tqdm(data, desc="data"):
                            if(opt.interpolate_ids[2] != -1):
                                ...
                                if(loop_through_weights):
                                    ...
                                    for weight_lambda in attr_range:
                            
                                        weight_lambda1 = weight_lambda if weight_lambda in attr1_range else weight_lambda1_prev
                                        weight_lambda2 = weight_lambda if weight_lambda in attr2_range else weight_lambda2_prev
                                        ...
                                        # save samples
                                        torchvision.utils.save_image(transforms.ToTensor()(image[0]), os.path.join(outpath_new,"original","./image_wo_attr_edit.jpg"))
                                        ...
                                        if not opt.skip_grid:
                                            ...
                                            for idx, one_grid in enumerate(grid):
                                                if(idx==0):
                                                    ...
                                                    #                         f'{opt.img_suffix[:40]}-{attr}.jpg'))
                                                    # save only the editted image
                                                    grid = 255. * grid[0].permute(0,2,3,1).cpu().numpy()
                                                    Image.fromarray(grid[-1].astype(np.uint8)).save(
                                                        os.path.join(outpath, attr, f'{grid_count:04}-{prompt_all[prompts_idx].replace(" ", "-")}_'
                                                                            f'{opt.img_suffix[:40]}_{attr}.jpg'))
