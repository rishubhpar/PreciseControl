...
def main():
    ...
    if config.model.params.personalization_config.params.test_mode == "image":
        ...
        delta_w_lambda = 1
        # attr_list = ["race_black", "race_white", "emotion_angry","emotion_surprise", "emotion_happy", "glasses", "gender", "race_yellow", "smile",
        #              "eyes_open", "beauty", "age", "emotion_fear", "emotion_disgust"]
        # attr_list = ["black", "white", "pose", "angry", "eyesclose", "hats", "bald", "disgust", "age80", "bang", "age70", "lights", "yellow","sad",
        #              "beard", "gender", "smile", "suprise", "eyeglasses", "eyeg"]
        attr_list = ["black", "angry", "eyesclose", "hats", "bald", "disgust", "bang", "age70", "yellow","sad",
                     "beard", "gender", "smile", "suprise", "eyeglasses", "eyeg"]
        # attr_list = ["face_age_70", "face_bald", "face_bang","face_beard", "face_eye_g", "face_hat", "face_lights", "face_smile", "open_close"]
        save_folder = 'delta_w_test'
        use_prompt_mixing = True
        steps_for_prompt_mixing = 10
        loop_through_weights = True
        do_one_identity = False
        opt.seed = 14
        sample_id = 3
    ...
    for i, attr in enumerate(attr_list):
        ...
        if(use_prompt_mixing):
            outpath1 = os.path.join(outpath, save_folder, attr, 'prompt_mixing{}'.format(steps_for_prompt_mixing))
        else:
            outpath1 = os.path.join(outpath, save_folder, attr, 'no_prompt_mixing')

        print("outpath", outpath1)
        print("output pathe exists", os.path.exists(outpath))
        if(not os.path.exists(outpath1)):
            print("creating outpath")
            os.makedirs(os.path.join(outpath1, 'gif'), exist_ok=True)

        grid_count = len(os.listdir(outpath1)) - 1
        outpath_new = os.path.join(outpath1, f"sample_{grid_count}")
                        ...
                        for prompts in tqdm(data, desc="data"):
                            if(opt.interpolate_ids[2] != -1):
                                ...
                                if(loop_through_weights):
                                    ...
                                    for weight_lambda in np.arange(-1.8, 2.4, 0.15):
