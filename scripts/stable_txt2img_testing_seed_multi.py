...
class e4e_wrapper():
    def __init__(self, checkpoint_path, mlp_e4e=False, ns=7, device='cuda'):
        self.checkpoint_path = checkpoint_path
        self.device = device
        self.num_of_styles = ns
        self.use_neutral_mlp = mlp_e4e
        self.load_e4e_with_decoder(device)
    ...
    def get_wlatents(self, x, is_cars=False):
        with torch.no_grad():
            ...
            orig_shape = codes[:,:self.num_of_styles,:].shape
            if(self.use_neutral_mlp):
                if(self.num_of_styles!=0):
                    style_weights = self.e4e_encoder.neutral_mlp(codes[:,:self.num_of_styles,:].reshape(codes.shape[0], -1))
                    l1_loss = torch.nn.L1Loss()(style_weights, torch.zeros_like(style_weights))
                    style_codes = torch.matmul(style_weights, self.e4e_encoder.attr_directions)
                    style_codes = style_codes.reshape(-1, 18, 512)
                    codes = codes + style_codes

            if codes.shape[1] == 18 and is_cars:
                codes = codes[:, :16, :]
        return codes

    def load_e4e_with_decoder(self, device='cuda'):
        if(self.use_neutral_mlp):
            import sys
            sys.path.append("/mnt/data/rishubh/sachi/encoder4editing")
            from models.psp_neutral import pSp
        else:
        ...
        if(self.use_neutral_mlp):
            print("Using e4e encoder with mlp layer")
            self.e4e_encoder = pSp(opts, use_neutral_mlp=self.use_neutral_mlp, num_of_styles=self.num_of_styles)
        else:
...
def main():
    ...
    if config.model.params.personalization_config.params.test_mode == "image":
        ...
        multi_sample_trans = transforms.Compose([
    ...
    e4e = e4e_wrapper("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/weights/encoder/e4e_ffhq_encode.pt")
    use_blurred = False
    # source_latent = e4e.get_wlatents(transforms.Resize((256,256))(pil_to_4d(source_exp,1).permute(0,3,1,2)))
    # neutral_source_latent = e4e_neutral.get_wlatents(transforms.Resize((256,256))(pil_to_4d(source_exp,1).permute(0,3,1,2)))
    img_folder_name = "lora_finetune_comparision_data/taylor_5"
    files_list = sorted(os.listdir("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/0000/".format(img_folder_name)))
    w_latent_avg = []
    for files in files_list:
        img = Image.open("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/0000/{}".format(img_folder_name,files)).convert("RGB")
        if(use_blurred):
            img = img.resize((32,32))
        img = multi_sample_trans(img).unsqueeze(0).to(device)
        w_latent_avg.append(e4e.get_wlatents(img))
        break
    w_latent_avg = torch.stack(w_latent_avg, dim=0)
    ...
    seed_list = range(1,2,1)
    two_ids = False
    multi_face = ["feynman1.jpg","8.jpeg","adele.jpg"]
    ...
    print("Images will be save in path : ", outpath)

    img_folder_name = "comparision"
    files_list = os.listdir("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/".format(img_folder_name))
    # files_list = ["00020_0_blurred_64.jpg", "00018_0_blurred_64.jpg", "00041_0_blurred_64.jpg"]
    # files_list = ["00020_0.jpg"] # 00050_0_1, 00055_0_1
    for l, img in enumerate(files_list):
        if(img.endswith(".jpg") or img.endswith(".png") or img.endswith(".jpeg")):
            ...
            # id1 = id1.resize((64,64))
            faces = pil_to_4d(id1.resize((32,32)))
            ...
            image_ori = {
                ...
                "delta_w": delta_w,
                "w_latents": w_latent_avg.repeat(batch_size, 1, 1)
            }
                                ...
                                if(opt.interpolate_ids[2] != -1):
                                    ...
                                    for seed in seed_list:
                                        ...
                                        if not opt.skip_grid:
                                            ...
                                            # data = pickle.load(f)
                                            id1 = id1.resize((32,32))
                                            input_img = np.array(id1.resize((512,512)))
