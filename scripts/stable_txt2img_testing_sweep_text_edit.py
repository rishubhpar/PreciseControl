...
def load_second_lora_weights(model, embedding_path, lora_finetuned_path, lora_scale=0.1, beta=1):
    lora_path = embedding_path.replace("embeddings_gs", "lora_params")
    lora_path = lora_path.split('/')
    lora_path[1] = lora_finetuned_path
    lora_finetuned_path = '/'.join(lora_path)
    ...
    monkeypatch_add_lora(model.model, torch.load(lora_finetuned_path), alpha=lora_scale, beta=1.0)
    return model
...
def main():
    ...
    # adding second id lora weights
    if(len(opt.lora2_path)!=0):
        model = load_second_lora_weights(model, opt.embedding_path, opt.lora2_path, opt.lora2_scale)
    else:
    ...
    if not opt.from_file:
        ...
        # temp_pos, temp_neg = "{}", "magzine, frame, tiled, repeated, split frame, multiple panel, \
    ...
    import json
    delta_w_dict = json.load(open("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/flame_delta_w_dict.json"))
    ...
    if(os.path.exists(os.path.join(outpath, text_attr))):
        print("creating outpath")
        # shutil.rmtree(os.path.join(outpath,"original"))
        shutil.rmtree(os.path.join(outpath,text_attr))
    os.makedirs(os.path.join(outpath, text_attr), exist_ok=True)
    grid_count = len(os.listdir(outpath)) if len(os.listdir(outpath)) != 0 else 0

    img_folder_name = "comparision"
    files_list = sorted(os.listdir("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/".format(img_folder_name)))
    ...
    for l, img in enumerate(files_list):
        if(img.endswith(".jpg") or img.endswith(".png") or img.endswith(".jpeg")):
            ...
            if(two_ids):
                ...
                for i in range(len(multi_face)):
                    id2 = Image.open("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/{}".format(img_folder_name, multi_face[i])).convert("RGB")
                    multi_faces_list.append(pil_to_4d(id2))
                # id2 = Image.open("/mnt/data/rishubh/sachi/CelebBasis_pstar_sd2/aug_images/{}/edited/{}".format("compton", "2.jpg")).convert("RGB")
        ...
        if(two_ids):
            if(attr is not None):
                delta_w1 = torch.tensor(delta_w_dict[attr], device=device).repeat(batch_size, 1, 1) * weight_lambda
            else:
                delta_w1 = torch.zeros_like(torch.tensor(delta_w_dict['smile'], device=device).repeat(batch_size, 1, 1))
            if(attr2 is not None):
                delta_w2 = torch.tensor(delta_w_dict[attr2], device=device).repeat(batch_size, 1, 1) * weight_lambda
            else:
        ...
        # image_ori = {
        #     "faces": faces if not two_ids else torch.cat([faces, faces2], dim=-1),
                                ...
                                if not opt.skip_grid:
                                    ...
                                    for idx, one_grid in enumerate(grid):
                                        if(idx==0):
                                            ...
                                            Image.fromarray(grid[sample_id+1].astype(np.uint8)).save(
                                                os.path.join(outpath, text_attr, f'{grid_count:04}-{prompt_all[prompts_idx].replace(" ", "-")}_'
                                                                    f'{opt.img_suffix[:40]}_{text_attr}.jpg'))
                                            grid_count += 1
